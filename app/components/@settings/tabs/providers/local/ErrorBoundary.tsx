      ...
      return (
        <div className={classNames('p-6 rounded-lg border border-red-500/20', 'bg-red-500/5 text-center')}>
          ...
          <h3 className="text-lg font-medium text-red-500 mb-2">
            {i18n.t('settings:providers.local.errorBoundary.title')}
          </h3>
          <p className="text-sm text-red-400 mb-4">{i18n.t('settings:providers.local.errorBoundary.description')}</p>
          <button
          ...
          >
            {i18n.t('settings:providers.local.errorBoundary.tryAgain')}
          </button>
          {process.env.NODE_ENV === 'development' && this.state.error && (
            <details className="mt-4 text-start">
              <summary className="cursor-pointer text-sm text-red-400 hover:text-red-300">
                {i18n.t('settings:providers.local.errorBoundary.errorDetails')}
              </summary>
