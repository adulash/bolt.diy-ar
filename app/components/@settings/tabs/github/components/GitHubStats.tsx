          ...
          <div className="flex items-center gap-2">
            {isLoading ? (
              <>
                ...
                <span className="text-bolt-elements-textSecondary">{t('github.stats.loading')}</span>
              </>
            ) : (
              <span className="text-bolt-elements-textSecondary">{t('github.stats.none')}</span>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-lg bg-bolt-elements-background dark:bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor hover:border-bolt-elements-borderColorActive/70 dark:hover:border-bolt-elements-borderColorActive/70 transition-all duration-200">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-bolt-elements-textPrimary">
                ...
                {isStale && <span className="text-bolt-elements-textTertiary ms-1">{t('github.stats.stale')}</span>}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
        ...
        <CollapsibleContent className="overflow-hidden">
          <div className="space-y-4 mt-4">
            ...
            <div className="mb-6">
              ...
              {stats.mostUsedLanguages && stats.mostUsedLanguages.length > 0 ? (
                <div className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    {stats.mostUsedLanguages.slice(0, 15).map(({ language, bytes, repos }) => (
                      <span
                        ...
                        title={t('github.stats.languageTooltip', {
                          language,
                  ...
                  <div className="text-xs text-bolt-elements-textSecondary">{t('github.stats.basedOnCodebase')}</div>
                </div>
              ) : (
                <div className="flex flex-wrap gap-2">
            ...
            <div className="mb-6 p-4 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
              <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{t('github.stats.overview')}</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {(stats.publicRepos || 0) + (stats.privateRepos || 0)}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{t('github.stats.totalRepositories')}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">{stats.totalBranches || 0}</div>
                  <div className="text-xs text-bolt-elements-textSecondary">{t('github.stats.totalBranches')}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {stats.organizations?.length || 0}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{t('github.stats.organizations')}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-bolt-elements-textPrimary">
                    {Object.keys(stats.languages).length}
                  </div>
                  <div className="text-xs text-bolt-elements-textSecondary">{t('github.stats.languagesUsed')}</div>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <h5 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">
                {t('connections.activitySummary')}
              </h5>
              <div className="grid grid-cols-4 gap-4">
                {[
                  {
                    label: t('github.stats.totalBranches'),
                    value: stats.totalBranches || 0,
                    icon: 'i-ph:git-branch',
                    iconColor: 'text-bolt-elements-icon-info',
                  },
                  {
                    label: t('connections.contributors'),
                  ...
                  {
                    label: t('connections.pullRequests'),
