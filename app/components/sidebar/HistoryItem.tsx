...
export function HistoryItem({
  ...
  return (
    ...
    >
      {selectionMode && (
        <div className="flex items-center me-2" onClick={(e) => e.stopPropagation()}>
          <Checkbox
            id={`select-${item.id}`}
            checked={isSelected}
      ...
      {editing ? (
        ...
        >
          <WithTooltip tooltip={currentDescription}>
            <span className="truncate pe-24">{currentDescription}</span>
          </WithTooltip>
          <div
            className={classNames(
              'absolute end-0 top-0 bottom-0 flex items-center bg-transparent px-2 transition-colors',
            )}
          >
            <div className="flex items-center gap-2.5 text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
              <ChatActionButton
                toolTipContent={t('history.export')}
              ...
              {onDuplicate && (
                <ChatActionButton
                  toolTipContent={t('history.duplicate')}
              ...
              <ChatActionButton
                toolTipContent={t('history.rename')}
              ...
              />
              <ChatActionButton
                toolTipContent={t('history.delete')}
